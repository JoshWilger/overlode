...
public class SpawnScrollingObjects : MonoBehaviour
{
    ...
    private void Update()
    {
        var rightHeight = Mathf.FloorToInt(transform.position.y + 15);
        if (rightHeight > highestY && objects.Count < 100)
        {
            for (int i = 0; i < rightHeight - highestY; i++)
            {
            ...
            highestY = Mathf.FloorToInt(transform.position.y + 15);
        }
        else if (rightHeight < spawnMe.transform.position.y && objects.Count > 0)
        {
            ...
            objects.Clear();
            highestY = Mathf.FloorToInt(transform.position.y);
        }
    }
}
