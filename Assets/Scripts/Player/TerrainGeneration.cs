...
public class TerrainGeneration : MonoBehaviour
{
    ...
    public const int WORLD_OFFSET = -3;
    ...
    private void Start()
    {
        ...
        {
            for (int x = WORLD_OFFSET; x < WIDTH + WORLD_OFFSET; x++)
            {
            ...
            if (!tilemaps[1].HasTile(new Vector3Int(x, -100)))
            {
                for (int y = 7; y < DEPTH && !tilemaps[1].HasTile(new Vector3Int(x, -y)); y++)
                {
                    for (int i = 0; i < earthquakeLayerAttempts; i++)
                    {
                        ...
                        var currentCoord = new Vector3Int(random > WIDTH + WORLD_OFFSET ? random - (WIDTH + WORLD_OFFSET) : random < WORLD_OFFSET ? random + WIDTH + WORLD_OFFSET : random, -y);
